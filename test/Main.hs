...
prop_extract alg Cycle{..} =
  ...
  where (mu, lambda, (pre, cyc)) = findCycleExtract alg cycF cycX0
...
prop_finite alg Cycle{..} =
  ...
  where xs = iterate cycF cycX0

tests :: TestTree
tests = testGroup "Data.FindCycle minimal tests"
  [
    let (name, alg) :| _ = totalAlgs in
    testProperty (name ++ " is correct")
      (prop_algCorrect alg)
  , testGroup "All total algorithms agree" [
      testProperty (refName ++ " ~ " ++ name) (prop_algsAgree refAlg alg)
      | let ((refName, refAlg) :| algs) = totalAlgs,
        (name, alg) <- algs
    ]
  , testGroup "Total agrees with minimalMu of partial" [
      testProperty (refName ++ " ~ minimalMu " ++ algName)
        (prop_algsAgree refAlg (minimalMu alg))
      | let ((refName, refAlg) :| _) = totalAlgs,
        (algName, alg) <- toList partialAlgs
    ]
  , testGroup "mu upper bound for partial" [
      testProperty name (prop_muUpperBound alg)
      | (name, alg) <- toList partialAlgs
    ]
  , testProperty "minimalMu is idempotent"
      (prop_algsAgree (minimalMu defAlg) (minimalMu (minimalMu defAlg)))
  , testGroup "Extractors agree" [
      testProperty (refName ++ " ~ " ++ rName)
        (prop_runnersAgree (refR defAlg) (r defAlg))
      | let ((refName, refR) :| rs) = extractors,
        (rName, r) <- rs
    ]
  , testProperty "findCycle agrees with extractors"
      (prop_runnersAgree
